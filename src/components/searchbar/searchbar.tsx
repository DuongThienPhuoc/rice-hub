...
const SearchBar: React.FC<SearchBarProps> = ({ onSearch, selectOptions }) => {
    ...
    return (
        <form className='bg-[#FFFFFF] flex rounded-lg' onSubmit={handleSearch}>
            <select className='px-2 py-2 text-white bg-[#1d1d1f] outline-none rounded-l-lg text-[14px]' name="searchField">
            ...
            />
            <button type="submit">
                <Image className='mr-2 min-w-[14px] min-h-[14px]' src={searchIcon} alt='search icon' width={20} height={20} />
            </button>
        </form>
    );
};
